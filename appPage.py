...
class AppPage(tk.Frame):
	...
	def clicked_add_new_btn(self):
		...
		self.virt_img = tk.PhotoImage(width=1, height=1)
		self.add_new_label = tk.Label(self.detail_add_new_header, text="Tambah Kontak Baru", font=("Arial", 30), width=frame_w, height=frame_h, image=self.virt_img, compound='c', bg="white")
		...
		for row in range(rows):
			...
			for column in range(columns):
				if column == 0:
					...
					entry = tk.Entry(self.detail_add_new_content, font=("Arial", 12), bg="white", textvariable=self.entry_update_contact_vars[row])
					sticky = "w"
					aRow.append(entry)
					entry.grid(row=row, column=column, sticky=sticky)
			self.table_info.append(aRow)
